...
def softmax(x):
    ...
    out = tf.exp(x)
    sum_x = tf.reduce_sum(out,reduction_indices = [1])
    sum_x = tf.reshape(sum_x,[tf.shape(sum_x)[0],1])
    out = out/sum_x
...
def cross_entropy_loss(y, yhat):
    ...
    cross_entropy = tf.reduce_sum(-tf.reduce_sum(tf.log(yhat)*y,reduction_indices=[1]))
    out = cross_entropy
    ### END YOUR CODE
