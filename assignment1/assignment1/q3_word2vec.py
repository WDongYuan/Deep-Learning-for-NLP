...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    ### YOUR CODE HERE
    target_vector = np.zeros(outputVectors.shape[0])
    target_vector[target] = 1

    softmax_out = softmax(np.dot(predicted,outputVectors.T))
    cost = np.sum(-target_vector*np.log(softmax_out))

    gradPred = np.dot(softmax_out-target_vector,outputVectors)
    grad = np.outer(softmax_out-target_vector,predicted)
    # raise NotImplementedError
    ### END YOUR CODE
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    in_vec = inputVectors[c_index]
    cost = 0
    # mypre_grad = np.zeros(inputVectors.shape[1])
    # myoth_grad = np.zeros(inputVectors.shape)
    gradIn = np.zeros(inputVectors.shape)
    gradOut = np.zeros(outputVectors.shape)
    ...
    # gradOut = np.zeros(outputVectors.shape)
    # softmax_out = softmax(np.dot(in_vec,inputVectors.T))
    # for cw in contextWords:
    #     cnt_index = tokens[cw]
    #     one_hot_vec = np.zeros(inputVectors.shape[0])
    #     one_hot_vec[cnt_index] = 1
    #     gradOut += np.outer(softmax_out- one_hot_vec,in_vec)
    ### END YOUR CODE
